...
function CriterionForm({
  ...
  const [rawValue, setRawValue] = useState(
    editTarget ? String(editTarget.value) : "",
  );
  const [level, setLevel] = useState<RiskLevel>(
    editTarget?.level ?? "HIGH",
  );
  const [touched, setTouched] = useState(false);

  const unit =
    CONDITION_FIELDS.find((f) => f.value === field)?.unit ??
    "원";
  ...
  const showError =
    touched && !result.valid && result.type !== "empty";
  const showDupError =
    ...
    >
                ...
                {!result.valid &&
                  result.type === "duplicate" && (
                    <p className="text-xs text-red-600 mt-0.5">
                      동일한 조건의 리스크 기준이 이미 등록되어
                      있습니다. 기존 기준을 수정하거나 다른
                      기준값을 입력하세요.
                    </p>
                  )}
              </div>
      ...
      <div className="px-5 py-3.5 border-t border-gray-200 bg-gray-50 flex items-center justify-end gap-3">
        ...
        >
          ...
          {editTarget ? "수정 저장" : "저장"}
          {!canSave && (
            <span className="text-xs opacity-70">(비활성)</span>
          )}
        </button>
      </div>
    </motion.div>
        ...
        <div className="px-6 py-5">
          <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 mb-4">
            ...
            >
              ...
              {formatValue(criterion.value, criterion.unit)}
            </p>
            <span
              className={`text-xs mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${criterion.level === "HIGH" ? "bg-red-50 text-red-700 border border-red-200" : "bg-amber-50 text-amber-700 border border-amber-200"}`}
            >
          ...
          <p className="text-sm text-gray-600 leading-relaxed">
            이 기준을 삭제하면 이후 제출되는 문서에 해당 조건이
            적용되지 않습니다. 정말 삭제하시겠습니까?
          </p>
        </div>
...
export function RiskCriteriaPage() {
  ...
  // 👉 [추가] 시뮬레이터 상태 관리 및 계산 로직
  const [simField, setSimField] =
    useState<ConditionField>("금액");
  ...
  const getSimulatedRisk = () => {
    ...
    const relevant = selectedForm.criteria.filter(
      (c) => c.field === simField,
    );
    ...
    for (const c of relevant) {
      ...
      else if (c.operator === "초과(>)")
        matched = val > c.value;
      else if (c.operator === "이하(≤)")
        matched = val <= c.value;
      else if (c.operator === "미만(<)")
        matched = val < c.value;
      else if (c.operator === "동일(=)")
        matched = val === c.value;
  ...
  const handleAdd = (
    data: Omit<RiskCriterion, "id" | "createdAt">,
  ) => {
    setForms((prev) =>
      prev.map((f) =>
        f.id !== selectedFormId
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2.5 mb-1.5">
              ...
              <span
                className="inline-flex items-center gap-1.5 text-xs bg-blue-600 text-white px-2.5 py-1 rounded-full shrink-0"
                style={{ fontWeight: 600 }}
              >
                💼 영업본부 부서 관리자
              </span>
            ...
            <p className="text-sm text-gray-500">
              양식별 고위험 판단 기준을 설정합니다. 기준이
              충족되면 해당 문서는 자동으로 고위험으로
              분류됩니다.
            </p>
          </div>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500">
              ...
              <span>
                ...
                <strong className="text-gray-700">
                  {selectedForm.criteria.length}개
                </strong>
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-[1fr_280px] gap-5 items-start">
                ...
                <div>
                  ...
                  {selectedForm.criteria.map((c, idx) => (
                    ...
                    >
                      ...
                      <span
                        className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border w-fit ${c.level === "HIGH" ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                      >
                        {c.level === "HIGH" ? "🔴" : "🟡"}{" "}
                        {c.level}
                      </span>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3 shadow-sm">
              ...
              <div className="space-y-2">
                <select
                  ...
                  className="w-full px-3 py-2 text-xs border border-gray-300 rounded-lg focus:outline-none focus:border-blue-400 bg-white"
                >
                  {CONDITION_FIELDS.map((f) => (
                    <option key={f.value} value={f.value}>
                      {f.value}
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs border border-gray-300 rounded-lg focus:outline-none focus:border-blue-400 transition-colors"
                />
              </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <h4
                className="text-gray-800 text-sm border-b border-gray-100 pb-2 mb-3"
                style={{ fontWeight: 600 }}
              >
                조건 적용 방식
              </h4>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              ...
              <div className="space-y-1.5">
                {forms.map((f) => (
                  ...
                  >
                    <span
                      className={`text-xs ${f.id === selectedFormId ? "text-blue-700" : "text-gray-600"}`}
                      style={{
                        fontWeight:
                          f.id === selectedFormId ? 600 : 400,
                      }}
                    >
                      {f.name}
                    </span>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${f.id === selectedFormId ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-500"}`}
                    >
                      {f.criteria.length}
                    </span>
                  </div>
