...
export function FormBuilderPage() {
  ...
  const handleSave = () => {
    setFormTitle(prev => prev.trim()); // 공백 제거 후 저장
    setSaving(true);
    setTimeout(() => {
      setSaving(false);
      if (!formTitle.trim()) {
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {selected ? (
                <>
                  ...
                  {selected.type === "table" && (
                    <>
                      <div className="space-y-1.5">
                        ...
                        {/* 💡 입력 및 수정을 자유롭게 하기 위해 join(",") 구조와 split(",") 구조로 단순화함 */}
                        <input
                          ...
                          className="w-full px-2.5 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:border-blue-400 font-medium"
                          placeholder="예: 품목명,수량,단가"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-xs text-gray-700 font-bold">
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold text-red-600 border border-red-200 bg-red-50 rounded-lg hover:bg-red-100 transition-colors"
                    >
                ...
                <div className="text-center text-gray-400 py-8">
                  ...
                  <p className="text-xs">
                    캔버스에서 필드를 선택하면 상세 속성을 설정할 수 있습니다.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className="flex-1 min-w-0 px-2.5 py-1.5 text-sm font-bold border border-gray-300 rounded bg-white focus:outline-none focus:border-red-400"
                  />
